...
To connect to {{site.data.keyword.messages-for-rabbitmq-gen2_full}}, you need some users and some connection strings. Connection Strings for your deployment are displayed on the _Dashboard Overview_, in the _Endpoints_ panel.
...
A {{site.data.keyword.messages-for-rabbitmq-gen2}} deployment is provisioned with an admin user, and after you [set the admin password](/docs/messages-for-rabbitmq-gen2?topic=messages-for-rabbitmq-gen2-user-management&interface=ui#user-management-set-admin-password-ui), you can use its credentials to connect to your deployment.
{: .tip}

## Credentials and connection strings for more users
{: #cred-connection-strings-additional-users}

Access to your {{site.data.keyword.messages-for-rabbitmq-gen2}} deployment is not limited to the root user. You can create users by using the _Service Credentials_ panel, the {{site.data.keyword.IBM_notm}} CLI, or through the {{site.data.keyword.IBM_notm}} {{site.data.keyword.databases-for}} API. 
